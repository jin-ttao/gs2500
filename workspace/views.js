...
function assumptions(vm) {
  ...
  return `<details class="assumptions"><summary>계산 입력·가정과 한계 보기</summary><div><p>30일 예상은 재현 가능한 로컬 집계 모델입니다. 3D는 기존 24시간 방문·이동 엔진을 별도로 실행합니다. 두 모델은 SKU·진열 위치·초기 재고를 공유하지만 같은 실행 기록은 아닙니다.</p><ul>${lines.map(row=>`<li>${escape(typeof row==='string'?row:JSON.stringify(row))}</li>`).join('')}</ul><p>JEV 실호출이나 실측 GS25 성과가 아닙니다. 시뮬레이션의 수익 차이를 실제 매출 상승이나 인과효과로 해석하지 마세요.</p></div></details>`;
}
...
function board(vm) {
  ...
  return heading('SIMULATION BOARD · 30 DAYS','여러 점포, 여러 선택을 한눈에','현재 진열과 후보의 30일 예상치를 비교하고, 기존 3D로 공간과 행동을 살펴보세요.',button('매대 목록','navigate',{data:{route:'bays'}}))+
    `<div class="board-status"><span class="status-dot ${vm.busy?'working':''}"></span><strong>${vm.busy?'30일 예상 계산 중':`${ready} / ${vm.bays.length} 매대 비교 준비`}</strong><span class="mono">${escape(vm.progressLabel ?? '동일 시드 · 동일 재고 · 같은 잠재 수요')}</span>${badge('로컬 집계 모델 · JEV 미호출')}</div>
    <div class="model-seam"><strong>두 가지 실험을 구분해 보여드립니다.</strong><span>숫자: 대상 24 SKU의 30일 가상 매출 · 움직임: 별도 24시간 3D 행동 실험. 점포 전체 매출이 아니며 3D의 하루 결제를 30일 예상치로 표시하지 않습니다.</span></div>
    <section id="world-board" class="world-board" aria-label="점포별 3D 시뮬레이션 전광판"><div class="world-placeholder"><span class="loading-ring"></span><p>기존 3D 공간을 연결하고 있습니다.</p></div></section>
    <div class="forecast-comparisons">${vm.bays.map(bay=>{
      const store=storeOf(vm,bay), result=findResult(vm,bay.storeId,bay.id);
      return `<section class="comparison-group"><div class="section-heading"><div><h2>${escape(store?.name)} <span>${escape(bay.name)}</span></h2><p class="micro mono">${escape(store?.id)} · ${escape(bay.id)} · 금액 단위 천원</p></div><span class="badge">현재 진열 ${money(result?.baseline?.revenue)} 천원 / 30일</span></div><div class="forecast-grid">${bay.candidates.map(candidate=>{
        const row=result?.candidates?.find(item=>item.candidateId===candidate.id);
        return `<article class="forecast-card"><div class="forecast-title"><span class="option-letter">${escape(candidate.id)}</span><h3>${escape(candidate.name)}</h3></div><div class="forecast-delta ${tone(row?.deltaPercent)}">${row?percent(row.deltaPercent):'—'}</div><p class="micro">현재 진열 대비 · 30일 예상</p><dl><div><dt>예상 결제매출</dt><dd>${money(row?.revenue)} <small>천원</small></dd></div><div><dt>예상 매출총이익</dt><dd>${money(row?.profit)} <small>천원</small></dd></div><div><dt>품절 수요 비율</dt><dd>${row?number(row.stockoutRate*100)+'%':'—'}</dd></div></dl><div class="option-meta"><span>약 ${number(candidate.minutes)}분</span><span>${candidate.orderSkus.length?`발주 검토 ${candidate.orderSkus.length} SKU`:'추가 발주 없음'}</span></div>${button(`${escape(candidate.id)}안 자세히 보기`,'select-candidate',{className:'button wide',data:dataFor(bay,candidate,'candidate'),disabled:!row})}</article>`;
      }).join('')}</div></section>`;
    }).join('')}</div><p class="disclaimer">예상 수익은 실제 관측 실적이 아닙니다. 소셜 신호·행사·수요는 합성 가정이며 전국 GS25 데이터를 수집한 결과가 아닙니다.</p>`;
}
